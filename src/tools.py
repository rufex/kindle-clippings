...
def append_to_files(dictio: dict, list_books_stored: list, log_file: Path):
    ...
    for book, highlights in dictio.items():
        ...
        with codecs.open(log_file, 'a', 'utf-8') as log:              # Open log-file. Append mode.
            log_txt = f'[{today}] Added {highlight_counter[book]:.0f} to existing {path_file.stem} \n'
            log.write(log_txt)
            print(log_txt)


def create_files(dictio: dict, output_folder_path: Path, log_file: Path):
    ...
    for book, highlights in dictio.items():
        ...
        with codecs.open(log_file, 'a', 'utf-8') as log:              # Open log-file. Append mode.
            log_txt = f'[{today}] New TXT File. Added {highlight_counter[book]:.0f} to existing {path_file.stem} \n'
            log.write(log_txt)
            print(log_txt)
